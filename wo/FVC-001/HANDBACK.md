...
- **The account-level plugin cache is already stale, and it's serving live rule content, not just tooling.** `story-board-b0` found and I independently verified: `~/Library/.../skills-plugin/.../skills/faceless-video-craft` auto-resynced to `metadata.version: 2.1.0` at 2026-09-03 00:43 — but that sync point predates `98ef6c7`. Its `references/restored-v1-rules.md` still carries the exact pre-fix bug that commit corrected: R1, R2, and R6 all cite `§Verification loop` instead of their real sections (`§9:16-native composition`, `§9:16-native composition`, `§Audio mastering`). `check_restored_citations.py` and `test_restored_citations.py` are absent from that copy entirely. Since `metadata.version` doesn't change between T1 and T9 (see §5), a version check can't detect this drift — anyone invoking the skill through the account-level path right now gets stale, mis-cited rule bodies, not the corrected ones. Fixing this needs the same ☐ upload step above, re-run after `98ef6c7`, not a code change.
