...
| [SkinBand](visual-components/skin-band/skinband-spike.html) | `skinband-spike.html` | A labelled two-layer skin cross-section — epidermis over dermis, a low-amplitude wavy surface line, a dashed boundary between them at a caller-chosen `boundary_frac` — that other actors compose within or beneath. `skin_band(w, h, boundary_frac=0.30)` has no randomness at all, unlike the actors that sit on it. | Unnumbered — confirmed catalog gap at the time: three uncatalogued one-off skin cross-sections already existed in the repo (`centella-tiger-grass`, `betaine-salicylate-gentle-bha`, `pdrn-cellular-science`), none harvested — the same "built five times, reused once" pattern named for `BarrierWall`. Harvested from `hyaluronic-acid-vs-filler` v2 (2026-09-03), used across four scenes to place each molecular form's actual depth. Carries one hard-won note: composing it twice for one panel (once inside a transform group, once again unshifted) produced two overlapping `EPIDERMIS` labels — caught on an extracted frame, invisible to `hyperframes check` |
