...
description: >
  ...
  are identical and sacred — the green/pink/cream editorial triad, Bricolage Grotesque at weight 500
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "Bricolage Grotesque", cqw: 1.56, weight: 400, lineHeight: 1.38 }
  body-card:{ fontFamily: "Bricolage Grotesque", cqw: 1.35, weight: 400, lineHeight: 1.34 }
  ...
  # — display ramp (Bricolage Grotesque weight 500, opsz, negative tracking) —
  title-card-sm:{ fontFamily: "Bricolage Grotesque", cqw: 2.9, weight: 500, lineHeight: 0.98, tracking: "-0.01em" }
  title-card:{ fontFamily: "Bricolage Grotesque", cqw: 3.5, weight: 500, lineHeight: 0.96, tracking: "-0.01em" }
  headline:{ fontFamily: "Bricolage Grotesque", cqw: 4.4, weight: 500, lineHeight: 1.0, tracking: "-0.02em" }
  headline-xl:{ fontFamily: "Bricolage Grotesque", cqw: 5.0, weight: 500, lineHeight: 0.96, tracking: "-0.02em" }
  display:{ fontFamily: "Bricolage Grotesque", cqw: 7.3, weight: 500, lineHeight: 1.02, tracking: "-0.02em" }
  display-hero:{ fontFamily: "Bricolage Grotesque", cqw: 11.5, weight: 500, lineHeight: 0.92, tracking: "-0.02em" }
  stat-figure:{ fontFamily: "Bricolage Grotesque", cqw: 11.5, weight: 500, lineHeight: 0.92, tracking: "-0.03em" }
  stat-figure-unit:{ fontFamily: "Bricolage Grotesque", cqw: 5.7, weight: 500, lineHeight: 0.92 }
  name:    { fontFamily: "Bricolage Grotesque", cqw: 2.3, weight: 600, lineHeight: 1.0 }
...
- **Fonts** — already set to **Bricolage Grotesque** (display) / **Bricolage Grotesque** (body); ignore any preset font name lingering in prose.
...
Penguin classic or a quiet annual report. One confident voice, **Bricolage Grotesque at weight 500**
...
- **Bricolage Grotesque weight 500** (opsz, negative tracking) for all display; **serif 400** body; **JetBrains Mono 500** uppercase chrome.
...
- **Squint** — one Bricolage Grotesque moment dominates at 3–6× its neighbor; the surface block centers the eye.
...
Two ramps. The **reading/chrome ramp** (Bricolage Grotesque **400** body 1.56cqw; JetBrains Mono labels
in px) carries copy + chrome; the **display ramp** (Bricolage Grotesque **500** `title-card-sm` 2.9cqw →
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar (mono label + monogram-circle), display-hero, body lede, footline. **Focal** a 2-line Bricolage Grotesque `display-hero` in pink (one word in cream), left. **Chrome** mono topbar; mono footline. **Accent** pink display + the monogram circle. **Silence** ~50%. **Fixed** serif 500, flat, monogram 2px pink. **Free** title, which word is cream. **Density** low.
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar, kpi-block (mono tag + 220px stat-figure + serif description), footline. **Focal** a Bricolage Grotesque `stat-figure` (~11.5cqw, +unit) in pink over a 2px pink rule. **Chrome** mono topbar + footline. **Accent** the pink figure. **Silence** ~55%. **Fixed** serif 500 figure, 2px accent rule, flat. **Free** the figure (from script), tag, description. **Density** low.

### 4 · Statement (quote · move: display serif · cream · left)

**Ground** `{colors.cream}`, generous pad. **Composes** topbar, display, name (600), caption-mono role. **Focal** a 2-line Bricolage Grotesque `display` (~7.3cqw) in green; a 600-weight attribution name + mono role beneath. **Chrome** mono topbar. **Accent** none — the serif carries it. **Silence** ~55%. **Fixed** serif 500 display + 600 name only, no italic. **Free** quote, name, role. **Density** low.
...
- Run every display in **Bricolage Grotesque weight 500**, opsz, negative-tracked, tight line-height; **body at 400**.
...
- **Type** — Bricolage Grotesque **500** display (opsz, negative-tracked) / **400** body; mono uppercase 0.08–0.18em; ≥1.4cqw floor.
...
- **Bricolage Grotesque (opsz 8..60) + JetBrains Mono via Google Fonts** — the optical-size axis is critical; a non-opsz fallback flattens the size-aware letterforms. CJK: LXGW WenKai (display) / Noto Serif SC (body) / Noto Sans Mono CJK (chrome).
...
## Snail Mucin Brand Notes (project-specific — read before authoring any frame)

**Hand-corrected color remap.** `build-frame.mjs`'s automated remix picked this
project's most-saturated brand colors (the warm shell/tan family) as the
single "accent" and repainted the preset's `green`/`green-deep`/`green-lite`
keys with that same shell hue — collapsing the deliberately muted moss-green
identity out of the palette entirely. The frontmatter above has been
hand-corrected to restore the real role split from the locked source
(`../snail-mucin-glass-skin/shot-plan.json` `palette_map`):

- `green` / `green-deep` / `green-lite` → the moss/sage family (`#2c4a34`
  ink_moss, `#0f2016` ink_dark, `#cddfb9` ground_3 pale sage) — the
  ingredient's real ground/gravity identity.
- `pink` / `pink-deep` → the warm shell/tan family (`#b9835a` shell,
  `#8a5a37` shell_deep) — the bottle-glass/snail-shell warmth. These keys are
  literally named "pink" in the inherited preset schema but carry no rose hue
  here; read them as "warm accent" / "warm accent deep."
- `cream` / `cream-2` → pearl (`#fbfaf4`) and pale sage-cream (`#e8efdd`).

**Motif-only colors — not in the 8-key palette above, used only in specific
frames.** The slime-trail seafoam→gold→lilac sequence (`#bdf0d4`,
`#f2e3b4`, `#e4c8ec`) and the bokeh fill `rgba(130,175,120,0.22)` are carried
over from the locked poster's dominant visual motif. They belong to Frame 1
(the hook's stroke-draw slime trail) and may reprise faintly in Frame 6
(outro). **They are a decorative motif, never a status/confidence color.**

**Success-color law (repo-wide, `catalog/README.md`).** This repo's rule is
that celadon/aqua tones read as "confirmed, no success color at any
level" — a claim with weak evidence must not look visually "worse" than a
well-evidenced one. The seafoam trail color above is chromatically close to
that family, so it is easy to misuse. To keep the rule checkable in this
project:

- The slime-trail seafoam/gold/lilac sequence is reserved for the hook motif
  (and its outro reprise) only — never used to color a CITATION chip, a
  benefit card, or any claim-adjacent element.
- Every `CITATION ⌞ ING-snail-mucin-S0NN ⌟` chip uses ONE designated
  treatment: `ink_moss` (`#2c4a34`) fill, `cream`/`pearl` mono text
  (JetBrains Mono, per `{typography.caption-mono}`), no seafoam, no
  confidence-scaled color. All four locked source ids get the identical
  treatment regardless of how strong their underlying evidence is.

**No brand mark anywhere.** Inherited from the locked poster and the sibling
10s piece (`shot-plan.json`'s `no_brand_mark`) — this video carries no logo,
wordmark, or URL lockup at any point, including the outro. The `monogram-circle`
component above is therefore unused in this project; the outro CTA is a
generic "subscribe" mono label with no identity stamp.

**CJK scope.** If the optional `蝸牛黏液精華` subhead (Frame 2) is kept, it
uses Noto Serif TC — scoped to that single text node only, per the
`red-ginseng-two-routes` precedent (`font-family: "Noto Sans KR"` /
`"Noto Serif TC"` applied to the specific `.cjk-subhead` element, never a
system-wide font swap). The rest of the type system stays Bricolage
Grotesque + JetBrains Mono as set in the frontmatter.
