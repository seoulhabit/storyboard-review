...
- **Not yet closed: a shipped/installed copy still can't verify itself.** `PROVENANCE.json` lives at the repo root, outside the packaged skill directory, so the guard exits 2 ("could not verify") from inside an unpacked zip or an installed cache — build-time enforcement (above) is the only place this is currently enforceable. `b0` has a further mechanism in progress (a staged `BUILD-ATTESTATION.json`, written only after the guard passes, re-verified by the shipped checker against the archive's pinned sha256 so a tampered archive can't launder a fresh matching attestation) — **explicitly uncommitted, pending Kim's go-ahead**, not part of this handback's attestation table.
