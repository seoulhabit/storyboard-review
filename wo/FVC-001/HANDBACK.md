...
- **The heading-containment guard already exists — it just doesn't run against distribution artifacts. Closed 2026-09-03.** `check_restored_citations.py` already asserted each rule body sits inside the section its `Source:` line names, not just "somewhere in the archive" — that check is exactly how R1/R2/R6 were caught in the first place (see the script's own docstring). The gap wasn't a missing check, it was that nothing ran it against `dist/*.zip` before upload — and this wasn't hypothetical: `story-board-b0` found the queued `dist/faceless-video-craft-2.1.0.zip` had been built 13 minutes *before* `98ef6c7` landed, so it shipped all three miscitations that commit fixed, one manual upload step from going out. Fixed and independently verified here: `claude-skills` commit `7325422` gates `release.sh` on the guard, accepting only exit 0 (not 2 — `--skip-freshness` can never reach 0, so this forces freshness to actually run, not silently degrade to consistency). The zip has been rebuilt; unpacked and confirmed its `restored-v1-rules.md` now cites `§9:16-native composition` (R1/R2) and `§Audio mastering` (R6), not `§Verification loop`. This closes the packaging/CI gap for build-time enforcement.
- **A shipped/installed copy can now verify itself too. Closed 2026-09-03.** `PROVENANCE.json` lives at the repo root, outside the packaged skill directory, so the guard used to exit 2 ("could not verify") from inside an unpacked zip or an installed cache — build-time enforcement (above) was the only place this was enforceable. Fixed at `5779d22`, verified here: the archive moved inside the packaged `faceless-video-craft/` directory; `release.sh` writes a `BUILD-ATTESTATION.json` into the staged copy only after the full guard (freshness included) passes against the source tree, re-deriving the sha from the staged bytes rather than trusting the earlier check; then, as its last step, it unpacks the zip it just built with the upstream repo deliberately hidden (`--upstream-repo /nonexistent`) and runs the packaged checker against itself, deleting the artifact outright if that fails. A tampered archive can't launder a fresh matching attestation — the sha pin binds the bytes independently, confirmed by the test suite's dedicated cases. Test suite is 25/25 (`python3 faceless-video-craft/tests/test_restored_citations.py` directly — `tests/run.sh`'s single aggregate line for this suite doesn't expand the count, a separate bug `story-board-b0` found and fixed, uncommitted pending Kim). `dist/faceless-video-craft-2.1.0.zip` is a different, larger artifact now (carries the archive plus the attestation) — if the claude.ai upload hasn't happened yet, take it fresh from `dist/` rather than anything built or circulated earlier tonight.
- **A Story Board history rewrite is a breaking change to claude-skills, not a local cleanup — surfaced but never triggered.** Kim considered an LFS history rewrite tonight (cancelled — the underlying pointer issue was already fixed at `d8fa838`, a normal commit, no SHAs moved). Had it run, `PROVENANCE.json`'s pin (`storyboard-review@606cb45`) would have gone stale, and `release.sh` would correctly start failing the citation guard on the next build until re-pinned — loud and correct, but real: any future Story Board rewrite blocks `claude-skills` releases, not just this repo's own history. Already-distributed copies (dist zips, installed caches) keep passing regardless, since they verify against their build-time attestation and sha256 pin rather than reaching upstream — correct, since the attestation records what was true when built, but it means a rewrite's effect on releases surfaces only at the *next* build, not immediately.
