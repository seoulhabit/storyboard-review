...
CAPTION_BAND_EXCLUDE = False    # exosome-label-problem has NO burned-in captions:
                                # the piece is type-carried and ships .srt/.vtt
                                # sidecars instead. Verified against THIS project's
                                # index.html, not inherited -- the runtime assertion
                                # below re-checks it on every run. Original note:
                                # this project has NO burned-in captions -- the on-screen
...
SAFE_TOP, SAFE_BOTTOM, SAFE_RIGHT, SAFE_LEFT = 192, 384, 162, 72
# ^ RE-DERIVED for exosome-label-problem 2026-09-02 from this project's own
#   assets/tokens/tokens.css and every scene's .stage padding. The copied
#   value was SAFE_LEFT = 0; this project actually reserves 72px on the left,
#   and leaving it at 0 puts an always-empty 72px strip inside the gridded
#   content box, which reads to the region check as a permanently empty cell.
