...
**Target skill:** `makemeavideo` 0.2.0
...
Five slots, one already answered. None of the remaining four has a default. The run does not start until all four are answered and Kim writes `approved`.

- **G0-1 — Engine reversal.** Confirm HyperFrames CLI becomes the build engine and Video Agent is removed from the skill. This is the reversal in §0 and needs to be affirmative, not assumed.
- **G0-2 — Imagery lane.** Confirm photographic and generated imagery is permitted in compositions as B-roll, permanently, under B-1. This closes the question WO-FVC-006 raised as G0-2.
- **G0-3 — Palette. `[BLOCKS T2]`** With G0-5 answered, the token layer is no longer one part of the design system — it *is* the design system, and the retrofit writes it into every catalog component. The three live palettes are: video design-system tokens (cream `#F4EDE3` / indigo `#26215C` / clay `#9C3A32`), the Nocturne luxury sheet (Ink / Ivory / Brass / Sage), and the pearl-white / blush / clinical-blue direction from WO-FVC-006. Pick one. Changing it later means re-running the retrofit across the library — cheap in compute, but every already-rendered plate goes stale. This is now the single highest-consequence open slot.
...
**R-6 — Catalog governance.** The catalog repo is cloned once, pinned to a SHA, and lives **outside the build path** (`vendor/hyperframes/`, gitignored from the build, SHA recorded in `video/system/catalog/manifest.json`). It is a build input, never a runtime dependency, and is never read at render time.

Catalog items are never consumed raw. Every item passes `retrofit_catalog.py` (T2) and is committed to `video/system/catalog/` as project source. The retrofitted file is the artifact. Catalog items are not the design system and never edit `video/system/` outside `catalog/`.

**R-7 — Engine.** `npx hyperframes render` locally. HeyGen supplies voice, images, and sound only. No `render_video`, no Video Agent, no avatars, no templates.
...
**R-12 — The design system is foundations, not components.** `video/system/` holds colour tokens, the two type families and their steps, the space scale, grid, safe areas, and motion tokens. Nothing else. The ten `sh-*` components move to `video/system/_archive/` with a dated note recording why; they are not deleted and not referenced by any composition. Recolouring the catalog into these tokens is how the channel keeps its identity — the brand now lives entirely in the token layer, which makes G0-3 load-bearing for every frame the channel ever renders.
...
Clone `https://github.com/heygen-com/hyperframes` into `vendor/hyperframes/`, pinned to a SHA, recorded in `video/system/catalog/manifest.json` alongside the CLI version in use.
...
3. Extract the voiceover. `vidiq_edit_media` with `op: extract_audio` accepts a YouTube URL. Target WAV, 48 kHz, stereo. Store as `videos/1-percent-trick-r2/vo/source-vo.wav`, hash it, and ledger the hash. **This file is the fixed variable of the whole experiment — it is never regenerated, re-synthesized, or re-levelled beyond a true-peak pass.**
...
Output: a token-native, brand-safe component in `video/system/catalog/`.

The script must:

1. **Strip remote font links.** Delete every `fonts.googleapis.com` / `fonts.gstatic.com` link and replace with the base64-inlined `@font-face` block for DejaVu Serif Bold and Archivo (800, 600) already used by the design system. A Google Fonts link previews correctly and silently falls back in the renderer — this is the documented brand-breaking failure and the reason this step is mechanical rather than optional.
...
3. **Pin the runtime.** Replace the jsdelivr GSAP CDN reference with the repo's vendored copy. No build-time network dependency.
4. **Record aspect.** Write the item's native dimensions and its T0 reframe verdict into the manifest.
5. **Emit a manifest entry** in `video/system/catalog/manifest.json`: item name, upstream version, retrofit script version, aspect, slots, and the hash of the retrofitted file.
...
These land in `video/system/catalog/` like everything else and get registry entries with fixed placement rules (lower third at first third, end card at final 4 seconds).
...
Rebuild `cGbokt_B_vE` as `videos/1-percent-trick-r2/`.

- Beat sheet: carried over from the original, unchanged. **Do not re-write the script.**
...
- Render: `npx hyperframes render`, locally, 1080×1920.
...
### T7 — Skill update to 0.2.0

Only after the pilot passes Gate C — the skill is updated from what actually worked, never from what was planned.

- S5–S7 rewritten against the HyperFrames CLI. Every `[SPIKE:n]` marker tied to Video Agent is removed, not left dangling.
- New S4.5 stage documented in `runbook.md`.
- `§H` rules: `H-1` (build spec generated, never typed) survives and now generates against the registry. `H-5` (per-scene chat fixes, capped) is deleted — it described a Video Agent affordance that no longer exists. Its replacement is a per-scene re-render cap.
- `B-1` added to `policy.md`.
- `R-6`, `R-7`, `R-9`, `R-10` added.
- `providers.yaml` updated: HeyGen roles narrowed to voice/image/sound.
- `CHANGELOG.md` records the engine reversal in plain terms.
- `sh-*` components moved to `video/system/_archive/` per R-12, with a dated note. `video/system/` is left holding foundations only. Verify no composition still references an archived component before closing.
- Canonical install is the `claude-skills` repo. Resolve the 0.2.0-vs-0.3.0 shared-checkout drift as part of this task, and verify both machines resolve the same version before closing.
...
- The PDRN story (`STORY-pdrn-left-the-clinic.md`). Currently written against Video Agent. It needs retargeting or explicit cancellation — flag it, do not silently retarget.
- Design-system safe-margin question, still open from WO-FVC-005.
- HeyGen connector authorization and the credit cap. Neither is required by this WO — T5 may reach Higgsfield, which is already authorized.
...
> Read `docs/wo/WO-FVC-007.md` in this repo and work from it. Run T0a and T0b only, then stop for Kim's Gate A ruling.
