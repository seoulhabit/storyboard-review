...
**Hand-corrected color remap (recurring bug — always verify).** `build-frame.mjs`'s
automated remix again picked this project's most-saturated brand color (the
warm shell/tan family) as the single "accent" and repainted every `green` /
`green-deep` / `green-lite` key with shell-family hues — the same failure
documented in the sibling project `../snail-mucin-truth/frame.md`. The
frontmatter above has been hand-corrected to the verified role split from the
locked source (`../snail-mucin-glass-skin/shot-plan.json` `palette_map`),
identical to the sibling's correction:
...
- `pink` / `pink-deep` → the warm shell/tan family (`#b9835a` shell,
  `#8a5a37` shell_deep) — the bottle-glass/snail-shell warmth. These keys
  inherit the name "pink" from the preset schema but carry no rose hue here;
  read them as "warm accent" / "warm accent deep."
- `cream` / `cream-2` → pearl (`#fbfaf4`) and pale sage-cream (`#e8efdd`).

**Continuity with `../snail-mucin-truth/`.** This is the second video built on
the same locked snail-mucin brand (Bricolage Grotesque + this moss/shell
palette). Keep the palette and type ramp identical across both videos — do
not introduce a third accent family or a different display face.
...
`#f2e3b4`, `#e4c8ec`) and the bokeh fill `rgba(130,175,120,0.22)` carry over
from the locked poster's dominant visual motif. This script's hook (macro
slime pulled apart by a spatula) and the mad-science chemistry-breakdown
frame are the natural home for this trail; it may reprise faintly in the
outro glow. **Decorative motif only — never a status/confidence color.**

**Success-color law (repo-wide, `catalog/README.md`).** Celadon/aqua tones
read repo-wide as "confirmed, no success color at any level" — a
weakly-evidenced claim must not look visually "worse" than a well-evidenced
one. The seafoam trail color is chromatically close to that family, so:

- Reserve the slime-trail seafoam/gold/lilac sequence for the hook motif (and
  its outro reprise) only — never on a CITATION chip, benefit card, or any
  claim-adjacent element.
- Every `CITATION ⌞ ING-snail-mucin-S0NN ⌟` chip (this video introduces
  `S005` origin-story and `S006` humectant-behavior alongside the reused
  `S001`–`S004`) uses the identical treatment: `ink_moss` (`#2c4a34`) fill,
  `cream`/`pearl` mono text (JetBrains Mono, `{typography.caption-mono}`), no
  seafoam, no confidence-scaled color — regardless of how strong the
  underlying evidence for that particular id is.

**No brand mark anywhere.** Inherited from the locked poster and both sibling
snail-mucin pieces (`no_brand_mark` in `shot-plan.json`) — no logo, wordmark,
or URL lockup at any point, including the outro's "subscribe" beat. The
`monogram-circle` component is unused; the outro CTA is a generic mono
"subscribe" label with no identity stamp.
