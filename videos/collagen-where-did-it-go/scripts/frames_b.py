...
"""
    ...
    tl.fromTo(".b-win", { opacity:0, scale:0.6 }, { opacity:1, scale:1, duration:0.3, stagger:0.03, ease:EASE.arrive }, @w(building) + 0.5);
    tl.to("#b-door", { opacity:1, duration:0.3, ease:EASE.arrive }, @w(building) + 0.9);
    // the collagen: beams DRAW in with the word
    tl.to(".beam", { strokeDashoffset:0, duration:0.55, stagger:0.10, ease:EASE.wipe }, @w(structure) - 0.1);
    ...
    CUT_ORDER.forEach(function (k, i) {
      tl.to("#beam-" + k, { strokeDashoffset:640, opacity:0.25, duration:0.42, ease:EASE.wipe }, @w(cut) + i * 0.34);
    });
    ...
    tl.to("#cite-uv", { opacity:1, duration:0.3, ease:EASE.arrive }, @w(enzymes) + 0.3);
