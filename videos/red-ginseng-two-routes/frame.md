...
colors:
  # Superseded by the hanbang/apothecary pass — see the note under this block.
  ink: "#2B2420"
  cream: "#F8F1E4"
  tile: "#A9C29A"
  tile-strong: "#7A9670"
...
- **This preset's prose (code-diff panels, PR figures, contributor avatars) is inherited from the generic "code-editorial" source and was never rewritten for this ginseng video** — it never applied here and can be ignored. The `colors:` tokens above were updated in place to match the final art direction: cool navy/coral/teal → warm ink-brown/terracotta/sage, after two rounds of art direction (an "organic warm" pass, then a Hanbang/apothecary pass — concentric root-cross-section and water-ripple rings, overlapping sine-wave linework, a muted-gold seal treatment for 인삼, further-softened corners). Component/typography/shadow prose below (hairline elevation, EB Garamond/Inter/JetBrains Mono ramp, one-accent-per-frame discipline) still holds in spirit. For exact current values — including the two-hue dark-surface split (`navy` for the oral/systemic diagram, `navy-soft` for the topical/surface diagram) and per-element opacities — treat `compositions/frames/*.html` as ground truth, not this document.
