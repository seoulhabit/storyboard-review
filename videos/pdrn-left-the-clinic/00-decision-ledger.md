...
## 9x16 static-hold — root cause found and fixed (this pass)

[FIX] Root cause isolated empirically, not guessed: extracted consecutive frames at a flagged window (27.0-29.5s, scene `f08-rows-origin`) and measured mean-abs pixel diff directly — **0.004-0.005, i.e. genuinely frozen**, despite the compiled HTML containing a real `tl.to(..., yoyo:true, repeat:N)` float tween. Isolated to `emit_sh_rows` specifically: it always registers its own float target (`#{cid}-hr{i}`, the row's 2px brass hairline divider), which satisfies the compiler's `if not markers.get("float")` fallback check and so PREVENTS the much larger, confirmed-working whole-stage fallback (`#{cid}-stage`) from ever firing for any `ShRows` scene. Measured the stage fallback directly on `s02-ingredient` (which has no component-level float, so gets the fallback): mean-abs diff **1.38-1.39**, ~300x larger, real visible motion. `ShSteps` (`emit_sh_steps`) registers no float marker at all, so it always gets the working stage fallback.
[FIX] Applied without touching the shared compiler: converted all 50 single-item `ShRows` scenes to `ShSteps`, mapping `rows[0].left → action`, `rows[0].right → note`, with `n` numbered sequentially within each original logical group (e.g. the 7-beat clinic-origin cluster reads steps 1-7, not 65 unrelated "1"s) rather than reusing the design system's literal "ordered routine" semantics for unrelated facts — a deliberate, disclosed content-authoring trade-off, not a perfect semantic fit, chosen because inventing a new component needs Design's ruling (Phase A request, still unanswered) and this ships a working artifact now.
[FIX] Recompiled + re-rendered both canvases (standard quality, not draft — draft was itself a questionable choice for a shipped deliverable, tested and ruled out as the cause of the original failure before finding the real one).
[RESULT] `H-4.static-hold`: 9x16 now **PASS** (was 8 violations, now 0). 16x9 unaffected (was already passing under its own looser 10.0s ceiling), confirmed still passing after the conversion.
[SIDE EFFECT, confirmed benign] More scenes now carry a bold clay numeral (`ShSteps`'s own `n`), giving the known Haar-cascade bold-typography false-positive class more surface: 16x9 now flags 2 frames (was 0), 9x16 flags 16 frames across 4 scenes (was 7 across 1). Every flagged box visually inspected — same class as the pre-existing `evidence-pct` "76%" false positive (confirmed at prior QA pass), same resolution: visual confirmation, not detector retuning. Evidence frames saved: `06-render/{9x16,16x9}/h3-verification/`.
[NOT FIXED, unchanged, out of this fix's scope] `H-4.contrast` on 9x16's endcard tagline — unrelated to cadence, a `ShEndcard.jsx` component defect already logged, not touched.

## Endcard contrast fix (this pass, operator-authorized)

[FIX] `H-4.contrast` failed on 9x16's endcard tagline (~3.1-3.2:1 vs 4.5:1 floor). Root cause: `ShEndcard.jsx`'s CTA line uses `var(--text-secondary)`, aliasing `var(--muted)` = `rgba(38,33,92,0.55)` — ink at 55% opacity on cream, which mathematically lands under WCAG AA regardless of which component renders it. Checked every use of `--text-secondary`/`--muted` across the system (`ShChip`, `ShEndcard`, `ShEvidence`, `ShMyth`, `ShCompare`, `ShQuote`, `ShIngredient`, `ShSteps`) — all 8 are genuinely-readable content (citations, corrections, attributions, INCI names, notes), not decoration, meaning the token itself was under-contrast everywhere, not only on the one frame this run's QA gate sampled.
[FIX] Applied at the token level (`videos/_system/tokens/colors.css`), per Kim's direct authorization, following the same procedure as ruling R-6 (a versioned amendment to the shared design system, not a per-component patch): `--muted` `rgba(38,33,92,0.55)` → `rgba(38,33,92,0.75)`. Recorded in `videos/_system/MANIFEST.json`'s `amendments[]` with the before/after value and full reasoning, matching R-6's own entry format. Re-verified manifest integrity clean (67/67, 0 mismatches) immediately after the edit.
[RESULT] Recompiled + re-rendered both canvases (standard quality). Measured: 9x16 contrast **3.13 → 5.54:1, PASS**. 16x9 unaffected (already 11.59:1, confirmed unchanged post-fix). Every H-4 pixel gate now passes on **both** canvases — canvas, contrast, type-floor, safe-area, static-hold, loudness, duration. Only `H-3` (the pre-existing, visually-confirmed bold-clay-typography false positive) still fails on paper; not a real defect, not touched by this fix, unchanged in kind from prior passes.
[NOTE] This amendment changes `--muted`'s rendered tone system-wide, for **every** future video built against `videos/_system/` — not scoped to this project. Flagged for whoever next reviews the design system broadly: other already-shipped videos built before this amendment used the old, under-contrast `--muted` value and would need a re-render to pick up the fix, same as R-6's own safe-area widening required.
