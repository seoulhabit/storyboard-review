...
- **The thumbnail is regenerated for this cut.** `assets/thumbnail/` now holds
  five candidates drawn from the 2:14.90 build; the shipped
  `thumbnail-final.png` is the climax flood at 101.60s. The decision was made at
  **120×67** — the size a thumbnail is browsed at — where it is the only
  candidate whose text survives at all; at 256×144 four of the five still read,
  which is why the old process, judging only there, picked differently. The
  generator now writes the shipped file from a `WINNER` constant rather than
  leaving a human to copy it. See `assets/thumbnail/README.md`.
